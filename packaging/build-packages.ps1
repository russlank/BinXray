...
<#
.SYNOPSIS
Builds BinXray release packages (portable EXE copy, MSI, and setup bundle EXE).
...
.EXAMPLE
.\packaging\build-packages.ps1 -AppExePath .\build\bin\BinXray.exe -Version 0.1.0

.EXAMPLE
.\packaging\build-packages.ps1 `
  -AppExePath .\build\bin\BinXray.exe `
  -Version 0.1.0 `
  -SignArtifacts `
...
$iconPath = Join-Path $repoRoot "src\BinXray\Resources\BinXray.ico"
...
$portableExePath = Join-Path $resolvedOutput "BinXray-$Version-x64.exe"
Copy-Item $resolvedExe $portableExePath -Force

$msiPath = Join-Path $resolvedOutput "BinXray-$Version-x64.msi"
$bundlePath = Join-Path $resolvedOutput "BinXray-$Version-x64-setup.exe"
